...
2. アプリケーションでコンポーネントを使用します：
   ...
   />
   ```
   このコンポーネントは input 要素のすべてのプロパティを転送するため、react-hook-form などのフォームライブラリと組み合わせて状態管理が可能です。
   ```jsx
...
### カスタム入力を使う
1. カスタム入力用の高階コンポーネントをインポートします：
   ```javascript
   import { withDateInputFeature } from 'keyboard-date-input';
   ```
2. カスタム入力コンポーネントに日付入力機能を付与します：
   ```jsx
   const DateInput = withDateInputFeature(YourInputComponent);
   ```
3. デコレートしたコンポーネントをアプリケーションで使用します：
   ```jsx
   <DateInput
     value={dateValue}
...
- `withDateInputFeature`: カスタム入力コンポーネントに日付入力機能を付与する高階コンポーネント。
  - `Component`: ラップするカスタム入力コンポーネント。
  - 日付入力機能を持つ新しいコンポーネントを返します。
- `withMonthInputFeature`: カスタム入力コンポーネントに月入力機能を付与する高階コンポーネント。
  - `Component`: ラップするカスタム入力コンポーネント。
  - 月入力機能を持つ新しいコンポーネントを返します。
...
- 1文字目
  ...
  - 3〜9の数字が入力された場合
    - 年の末尾に"/"がない場合は自動挿入する
    - 入力された数値に対して0パディングを行う。("yyyy/03","yyyy/09"など)
    - 日の入力に移行する
  - 1が入力された場合
    - 年の末尾に"/"がない場合は自動挿入する
    - 入力された数値に対して0パディングを行う。（"yyyy/01"となる）
...
- 1文字目
  ...
  - 1-9の数値が入力された場合
    - 月の末尾に"/"がない場合は自動挿入する
    - 自動的に0パディングを行う。("yyyy/mm/01","yyyy/mm/09"など)
- ２文字目
...
- 3〜9の数字が入力された場合
  - 入力された数値に対して0パディングを行ったものを月の値とする
##### バックスペースによる削除
